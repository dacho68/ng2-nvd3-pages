  ...
  candlestickBarChart: {
    chart:{
      ...
      xAxis: {
        ...
        tickFormat: function(d:any) {
          var dd = +new Date();
          return d3.time.format('%x')(new Date(dd - (20000 * 86400000) + (d * 86400000)));
        },
        showMaxMin: false
      },
      yAxis: {
...
function generateDataScatter(groups:any, points:any) {
  var data = [],
...
function generateDataMultiBar() {
  return stream_layers(3,50+Math.random()*50,.1).map(function(data:any, i:number) {
    return {
      key: 'Stream' + i,
      values: data
    };
